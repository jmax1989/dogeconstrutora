...
import { getPickTargets, stepX, stepZ, setGroupHighlight } from './geometry.js';
...
function applyHover(g){
  ...
  const curHover = State.__HOVER_GROUP__;
  const curSel   = State.__SEL_GROUP__;

  // se já é o mesmo, nada a fazer
  if (curHover === g) return false;

  // remove hover anterior (sem mexer na seleção)
  if (curHover && curHover !== curSel){
    setGroupHighlight(curHover, 'none');
  }

  // aplica hover novo, exceto se for o selecionado (seleção tem prioridade)
  if (g && g !== curSel){
